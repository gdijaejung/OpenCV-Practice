...
bool cDxCapture::Init(const int width, const int height)
{
	// Initialize COM
...
HRESULT cDxCapture::CaptureVideo()
{
	HRESULT hr;
...
HRESULT cDxCapture::FindCaptureDevice(IBaseFilter ** ppSrcFilter)
{
	HRESULT hr = S_OK;
...
void cDxCapture::WriteBitmapCurrentBuffer(const char* fileName)
{
	long cbBuffer;
...
IplImage* cDxCapture::GetBufferToImage()
{
	if (!m_iplImage)
	{
	...
	long size;
	BYTE* buffer = GetCurrentBuffer(size);
	if (!buffer)
		return m_iplImage;

...
void VideoCapture::SetSampleGrabber()
{
	...
	//IAMStreamConfig * pConfig = NULL;         // 미디어 타입을 구하거나 설정하는 인터페이스

	//hr = capture->FindInterface(NULL, NULL, samplegrabberfilter, IID_IAMStreamConfig, (void**)&pConfig);

	//AM_MEDIA_TYPE * pCurMediaType = NULL;    // 현재 미디어 타입을 가리킬 포인터
	//hr = pConfig->GetFormat(&pCurMediaType);

	//if (MEDIATYPE_Video == pCurMediaType->majortype)
	//{
	//	if (FORMAT_VideoInfo == pCurMediaType->formattype)
	//	{
	//		VIDEOINFOHEADER *pVih = (VIDEOINFOHEADER*)pCurMediaType->pbFormat;
	//		pVih->bmiHeader.biWidth = 1280;       // 원하는 해상도 중 너비 값을 대입합니다.
	//		pVih->bmiHeader.biHeight = 720;      // 원하는 해상도 중 높이 값을 대입합니다.
	//		pVih->bmiHeader.biSizeImage = DIBSIZE(pVih->bmiHeader);

	//		// (해상도가 바뀌면, 프레임의 크기도 바뀌므로 biSizeImage도 바꿔줘야 합니다.)



	//		// 변경된 미디어 타입을 다시 설정합니다.
	//		hr = pConfig->SetFormat(pCurMediaType);
	//		// 미디어 타입이 설정되지 않으면, 이곳에 오류를 처리해야 합니다.
	//	}
	//}
...
HRESULT VideoCapture::CallbackHandler::SampleCB(double time, IMediaSample *sample)
{
	...
	hr = sample->GetPointer(&pBmpBuffer);
	...
	callback(pBmpBuffer, sample->GetActualDataLength(), bitpixel);

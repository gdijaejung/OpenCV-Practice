...
// 카메라 회전을 인식한다.
void cRotateCamera::DetectCameraRotate(cCamera &camera0, const Point &origPoint, const Mat &skewTransform)
{
	int thresholdValue = g_config.m_binaryThreshold; // 임계값(Threshold) 설정
	int sensitive = g_config.m_rotateSensitivity;

	cLineContour origContours(origPoint, 20);
	cRectContour target(origPoint, 10);
	...
	while (1)
	{
		...
		if (!keypoints.empty())
		{
			if (keypoints.size() == 4)
			{
				...
				cLineContour detectRect(keypoints);

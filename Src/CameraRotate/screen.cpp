...
void cScreen::DetectScreen(cCamera &camera0, const float minScreenArea)
{
	...
	while (1) {
		...
		// 카메라 영상을 이진화 한다.
		if (!binImage) {
			binImage = cvCreateImage(camera0.Resolution(), IPL_DEPTH_8U, 1); // 흑백 이미지 생성
			binOutput = cvCreateImage(camera0.Resolution(), IPL_DEPTH_8U, 1); // 흑백 이미지 생성
		}
...
// 포인트를 출력한다.
void cScreen::ShowPoints(const vector<cv::Point2f> &points, const cv::Scalar &color, 
...
// color=Scalar(0,0,0), isClear = false, bgColor=cv::Scalar(255, 255, 255)
{
	...
	{
		ellipse(m_screen, RotatedRect(pt, Size2f(20,20), 0), color, CV_FILLED);
	}
	imshow("Screen", m_screen);
}

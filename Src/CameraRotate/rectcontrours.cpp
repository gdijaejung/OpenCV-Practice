...
#include "global.h"
#include "rectcontrours.h"


using namespace cv;


cRectContours::cRectContours() :
	m_contours(4)
{

}

cRectContours::cRectContours(const vector<cv::Point> &contours) :
	m_contours(4)
{
	Init(contours);
}

cRectContours::cRectContours(const vector<cv::KeyPoint> &contours) :
	m_contours(4)
{
	Init(contours);
}
...
// 초기화.
bool cRectContours::Init(const vector<cv::Point> &contours)
{
...
// 초기화.
bool cRectContours::Init(const vector<cv::KeyPoint> &keypoints)
{
	if (keypoints.size() < 4)
		return false;
...
// 3 -------- 2
//
bool cRectContours::Init(const cv::Point &center, const float size)
{
...
// 박스 출력.
void cRectContours::Draw(cv::Mat &dst, const cv::Scalar &color, const int thickness)
...
// 사각형의 중점을 리턴한다.
Point cRectContours::Center() const
{
	Point center;
...
// 사각형의 중점을 중심으로 스케일한다.
void cRectContours::ScaleCenter(const float scale)
{
